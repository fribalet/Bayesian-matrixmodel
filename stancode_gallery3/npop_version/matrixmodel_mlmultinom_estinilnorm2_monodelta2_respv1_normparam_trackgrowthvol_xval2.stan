...
transformed data {
    ...
    //for (ipop in 1:npop){
    //    alpha_pop[ipop] = 1.0 * ipop;
    //}
}
parameters {
    simplex[npop] pop_fraction;
    simplex[m-j+1] delta_incr[npop];
    real<lower=0, upper=1.0/dt_days> delta_max[npop];
    real<lower=0,upper=1.0/dt_norm> gamma_max[npop];
    real<lower=0,upper=1.0/dt_norm> rho_max[npop];
    real<lower=0, upper=5000> E_star[npop];
    real<lower=1e-10> sigma;
    simplex[m] theta[nt_obs];
    simplex[m] w_ini[npop];  // initial conditions
}
transformed parameters {
    real divrate[npop];
    real<lower=0, upper=1.0/dt_days> delta[npop,m-j+1];
    ...
    real<lower=0> cell_count[nt];       // record relative cell count for each time step
    {
        ...
        mod_obspos = rep_matrix(0.0, m, nt_obs);

        for (ipop in 1:npop){
        // populate delta using delta_incr
        delta[ipop,1] = delta_incr[ipop,1] * delta_max[ipop];
        for (i in 1:m-j){
            delta[ipop,i+1] = delta[ipop,i] + delta_incr[ipop,i+1] * delta_max[ipop];
        }
        ...
        for (it in 1:nt){ // time-stepping loop
            ...
            if (it == it_obs[ito]){
                mod_obspos[:,ito] += pop_fraction[ipop] * w_curr;
                ito += 1;
                if (ito > nt_obs){
            ...
            gamma = gamma_max[ipop] * dt_norm * (1.0 - exp(-E[it]/E_star[ipop]));
            gamma_sat = gamma_max[ipop] * dt_norm;
            rho = rho_max[ipop] * dt_norm;
            ...
            for (i in 1:m){ // size-class loop
                // compute delta_i
                if (i >= j){
                    delta_i = delta[ipop,i-j+1] * dt_days;
                }
        ...
        divrate[ipop] = log(sum(w_curr))*60*24/(nt*dt); // daily division rate
        }
    }
}
model {
    ...
    pop_fraction ~ dirichlet(alpha_pop);

    // priors
    for (ipop in 1:npop){
        //delta_max[ipop] ~ normal(100.0, 100.0) T[0, 1.0/dt_days];
        gamma_max[ipop] ~ normal(10.0, 10.0) T[0, 1.0/dt_norm];
        rho_max[ipop] ~ normal(3.0, 10.0) T[0, 1.0/dt_norm];
        E_star[ipop] ~ normal(1000.0,1000.0) T[0,];
    }
    delta_max[1] ~ normal(100.0, 100.0) T[0, 1.0/dt_days];
    delta_max[2] ~ normal(30.0, 30.0) T[0, 1.0/dt_days];
