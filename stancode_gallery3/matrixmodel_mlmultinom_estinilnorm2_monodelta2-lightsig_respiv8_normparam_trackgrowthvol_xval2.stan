...
parameters {
    ...
    real<lower=-beta_max,upper=beta_max> betar;
    ...
    real<lower=0> cell_count[nt];       // record relative cell count for each time step 
    {
        ...
        if (beta > 0){
            for (i in 1:m){ // size-class loop
                sizelim_gamma[i] = exp(beta*(v_mid[i]-v_mid[m])/v_range);
            }
        } else {
            for (i in 1:m){ // size-class loop
                sizelim_gamma[i] = exp(beta*(v_mid[i]-v_mid[1])/v_range);
            }
        }
        if (betar > 0){
            for (i in 1:m){ // size-class loop
                sizelim_rho[i] = exp(betar*(v_mid[i]-v_mid[m])/v_range);
            }
        } else {
            for (i in 1:m){ // size-class loop
                sizelim_rho[i] = exp(betar*(v_mid[i]-v_mid[1])/v_range);
            }
        }
