    ...
    real<lower=0> cell_count[nt];       // record relative cell count for each time step 
    {
        ...
        if (exponent_gamma > 0){
            for (i in 1:m){ // size-class loop
                sizelim_gamma[i] = exp(exponent_gamma*(v_mid[i]-v_mid[m])/v_range);
            }
        } else {
            for (i in 1:m){ // size-class loop
                sizelim_gamma[i] = exp(exponent_gamma*(v_mid[i]-v_mid[1])/v_range);
            }
        }
        if (exponent_rho > 0){
            for (i in 1:m){ // size-class loop
                sizelim_rho[i] = exp(exponent_rho*(v_mid[i]-v_mid[m])/v_range);
            }
        } else {
            for (i in 1:m){ // size-class loop
                sizelim_rho[i] = exp(exponent_rho*(v_mid[i]-v_mid[1])/v_range);
            }
        }
        ...
        for (it in 1:nt){ // time-stepping loop
            ...
            for (i in 1:m){ // size-class loop
                ...
                gamma -= dt_norm * sizelim_rho[i] * rho_max;
                gamma_sat = dt_norm * sizelim_gamma[i] * gamma_max;
                gamma_sat -= dt_norm * sizelim_rho[i] * rho_max;
                if (gamma_sat < 0){
                    gamma_sat = 0.0;
                }
                if (gamma > 0){
