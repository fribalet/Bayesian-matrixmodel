<<<<<<< Updated upstream:stancode/matrixmodel_sigmoidaldelta.stan
data {
    // size variables
...
model {
    ...
    sigma ~ exponential(1000.0); // Seems really small
    ...
    for (it in 1:nt_obs){
        ...
        diff ~ normal(0.0, sigma) T[0,];
	// E sigma^2 == 1E-6 // really small
    }
    ...
    for (it in 1:nt_obs){
        ...
        for (iv in 1:m){
            /* diff += fabs(mod_obspos[iv,it] - obs[iv,it]); */
	  diff += pow(mod_obspos[iv,it] - obs[iv,it], 2.0);
        }
        diff ~ chisq(p);
    }
}
>>>>>>> Stashed changes:stan_code/matrixmodel_sigmoidaldelta.stan
