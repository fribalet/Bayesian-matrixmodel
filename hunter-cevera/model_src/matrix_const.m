function B=matrix_const_plateau(hr,Einterp,volbins,b,dmax,E_star,Ymax,dt,ts)
...
for t=1:(1/dt)      
     ...
     A(stasis_ind)=(1-delta)*(1-y(t+round(hr/dt)));         % stasis, the hr/dt part in the indexing is because eahc hour is broken up into 1/dt min segments for the irradiance spline
     A(m,m)=(1-delta(m));
                 
     A(growth_ind)=y(t+round(hr/dt))*(1-delta(1,1:m-1));    %growth on sub-diagonal
