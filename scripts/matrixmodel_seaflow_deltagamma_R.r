...
setwd("/Users/ribalet/Documents/DATA/Codes/Bayesian-matrixmodel/")
    ...
    matrix[m,nt_obs] mod_obspos;
    {
        ...
        for (it in 1:nt){ // time-stepping loop
            ...
            for (i in 1:m){ // size-class loop
                // compute gamma
                if (i < m){
                    gamma = gamma_max[i] * dt_days * (1.0 - exp(-E[it]/E_star));
                }
...
summary(mcmc,pars=c('delta_max','gamma_max','E_star','sigma'))$summary

post <- extract(mcmc)

pairs(data.frame(delta_max=post$delta_max,
                 gamma_max=post$gamma_max,
                 E_star=post$E_star,
                 sigma=post$sigma)[sample(1:length(post$delta_max),500),])
...
for(i in 1:length(slice_indices)){
    plot(v_ext,res[,slice_indices[i]],type='s',ylim=c(0,max(c(data$obs[,slice_indices[i]],res[,slice_indices[i]]))),xlab='',ylab='Proportion')
        lines(v_ext,data$obs[,slice_indices[i]],col='red',type="s")
