...
parameters {
    ...
    real<lower=0,upper=10> gamma_max;
...
transformed parameters {
    ...
    vector<lower=0>[m] w_ini;  // initial conditions 
    {
        ...
        delta_max[1] = delta_max_incr[1];
        for (i in 1:m-j){
            delta_max[i+1] = delta_max[i] + delta_max_incr[i+1];
        }
            ...
            for (i in 1:m){ // size-class loop
                ...
                if (i >= j){
                    if (E[it] < delta_lightthresh){
                        ...
                        delta_i = delta_max[i-j+1] * dt_days * exp(-((E[it]-delta_lightthresh)/(100.0*delta_lightsigma))^2);
                    }
                }
