...
parameters {
    ...
    real<lower=0.0, upper=1.0> gamma_size[m-1];
    real<lower=0.0, upper=1.0> rho_size[m-1];
    ...
    real<lower=0> cell_count[nt];       // record relative cell count for each time step
    {
        ...
        for (it in 1:nt){ // time-stepping loop
            ...
            for (i in 1:m){ // size-class loop
                // compute delta_i
                if (i >= j){
                    delta_i = tau_t * delta_max[iday] * delta[i-j+1] * dt_days;
                }
                // compute size-dependent gamma and rho
                if (i < m){
                    gamma = dt_norm * gamma_max[iday] * gamma_size[i] * (1.0 - exp(-E[it]/E_star[iday]));
                    gamma_sat = dt_norm * gamma_max[iday] * gamma_size[i];
                } else {
                    gamma = 0.0; // value is not used
                    gamma_sat = 0.0; // value is not used
                }
                if (i > 1){
                    rho = dt_norm * rho_max[iday] * rho_size[i-1];
                } else {
...
model {
    ...
    gamma_max_sigma ~ exponential(0.1);
